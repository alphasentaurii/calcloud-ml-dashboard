...
def make_weights(clf):
    x_weights = input_layer_weights(clf)
    h1_weights = dense_layer_weights(clf, 'h2', 'h1')
    h2_weights = dense_layer_weights(clf, 'h3', 'h2')
    h3_weights = dense_layer_weights(clf, 'h4', 'h3')
    h4_weights = dense_layer_weights(clf, 'h5', 'h4')
    h5_weights = dense_layer_weights(clf, 'h6', 'h5')
    h6_weights = dense_layer_weights(clf, 'y', 'h6')
...
input_weights = input_layer_weights(clf) #delete
weights = make_weights(clf)
...
# for parent in nodes:
#     if parent['data']['id'] == 'inputs':
#         parent['classes'] = 'inputLayer'
#     elif parent['data']['id'] == 'outputs':
#         parent['classes'] = 'outputLayer'
#     else:
#         parent['classes']
...
def single_neuron(layer_num, input_arr, neuron):
    ...
    # get bias values
    bias = np.array(clf.layers[layer_num].bias)[neuron]
    WX = weights * input_arr
    relus = []
    for x in WX:
        relus.append(np.max([0, x]))
    n = np.sum([r+bias for r in relus])
